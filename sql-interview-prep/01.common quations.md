...
A constraint is a restriction placed on data in a table to make sure only valid data is stored
...
| ---------------------- | ------------------------------------------ | ------------------------------------------------------ |
| **Purpose**            | Filters **rows (records)** before grouping | Filters **groups (aggregated results)** after grouping |
| **Works with**         | Normal conditions on columns               | Aggregate functions (like `COUNT`, `SUM`, `AVG`)       |
| **Used with GROUP BY** | Cannot be used with aggregates             | Must be used with aggregates after `GROUP BY`          |
| **Execution Order**    | Applied **before GROUP BY**                | Applied **after GROUP BY**                             |
| **Example Usage**      | `WHERE age > 18`                           | `HAVING COUNT(*) > 5`                                  |


## Local Variables vs Global Variables in SQL
| Feature        | **Local Variable**                                | **Global Variable**                       |
| -------------- | ------------------------------------------------- | ----------------------------------------- |
| **Scope**      | Only inside function/procedure/block              | Available throughout the session/server   |
| **Lifetime**   | Created when block starts, destroyed when it ends | Exists as long as session/server runs     |
| **Defined by** | User (you declare it)                             | System (predefined)                       |
| **Prefix**     | `@` (SQL Server), none in PL/SQL                  | `@@` (SQL Server), `@@`/`GLOBAL` in MySQL |
| **Example**    | `DECLARE @count INT`                              | `SELECT @@VERSION`                        |

## What is Normalization?
- Normalization is the process of organizing data in a database to reduce redundancy (repeated data) and improve data integrity (consistency).
- It means breaking a big table into smaller related tables and linking them with relationships (primary key–foreign key)
  1] 1NF (First Normal Form)
  - Each column must have atomic (indivisible) values.
  - No repeating groups or arrays.
  - Example: Instead of storing multiple phone numbers in one column, create separate rows or a new table.
  
  2] 2NF (Second Normal Form)
  - Table must be in 1NF.
  - No partial dependency (non-key column should not depend on part of a composite primary key).

  3] 3NF (Third Normal Form)
  - Table must be in 2NF.
  - No transitive dependency (non-key column should depend only on the primary key, not on another non-key column).
